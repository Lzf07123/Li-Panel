...
- 2026-08-21 页脚完善：ICP/公安备案条目与 `·` 分隔、图标缺失显示「备/公」占位、`.env.example` 补前端备案变量。
- 2026-08-21 备案后台同步：`site_settings` 新增备案链接/图标/公安备案字段，页脚运行时读取后台配置（`brand.ts` 仅回退默认值），设置页可改并即时生效。
- 2026-08-21 个人设置比例统一：输入/选择同高、按钮统一 h-8；版式改为与其它设置页一致的全宽卡片 + 三列网格。
