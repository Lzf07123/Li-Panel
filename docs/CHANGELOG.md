...
### 存活检测提速：全并发探测 + 分批显示（2026-08-22）

- **根因**：上一轮「分批探测」改为每批 6 个串行发起（批间等上一批全部完成），链接多/站点慢时总耗时 = 批数 × 最慢单批，最坏可达数十秒，实测太慢。
- **修复**：探测恢复**全并发**（所有目标同时发起，浏览器连接池自然排队），总耗时 ≈ 最慢单个（≤5s）；「分批显示」保留——按完成计数每 6 个凑一批回调进度「检测中 {done}/{total} 批」，状态点仍逐条流式点亮（首个快链接约 100ms 出现）。
- **手动按钮不再等服务端**：登录用户点击后客户端探测完成即收尾（≤5s），服务端仅后台刷新历史/趋势/通知；访客无客户端探测，仍等服务端公开状态刷新。
- **测试隔离修复**：`_refresh_last`（30s 强制刷新节流）是模块级状态，`test_health.py` 的 autouse fixture 此前未清除，导致顺序性 flake（`test_health_refresh_bypasses_cache` 偶发失败）；现跨用例清除。
- 验证：前端 vitest 40 passed（更新 2 条：全并发、按完成计数分批回调）；tsc / vite build / oxlint 通过；后端 pytest 212 passed。
